...
3. 로컬 개발용 `.env.local`에도 같은 값을 저장한다.
4. `GET /api/naver/cafe-search?query=검색어` 서버 라우트에서 카페글 검색 API를 호출한다.
5. 응답의 `cafeurl`이 감시 대상 카페와 일치하는 결과만 우선 반환한다.
6. `npm run fetch:cafe`를 실행하면 키워드 묶음을 수동으로 검색하고 `src/data/naver-cafe-results.ts`에 저장한다.
7. 사이트의 정보 보드는 저장된 공개 검색 결과를 보여준다.
...
```bash
curl "http://localhost:3000/api/naver/cafe-search?query=%EB%AA%A8%EB%8D%B8Y%20%EC%8D%AC%ED%8C%85"
```
