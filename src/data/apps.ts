...
export const coreApps: AppItem[] = [
  ...
  },
  {
    ...
    when: "첫 장거리 이동 전",
    note: "공식 데이터가 '운영중'이어도 실제로는 고장인 경우가 있다. 그 간극을 메우는 게 이 앱의 값어치다. 운영사는 소프트베리이고 별도 웹사이트가 없어 앱스토어로 연결한다.",
...
// 국내 내비를 차량 화면에 띄우는 방법(안드로이드 오토 미러링 계열)은 전부 비공식이고,
// 대표 도구였던 WebAA는 공식 사이트(webaa.dev) 도메인이 만료돼 2026-08-14 기준 접속되지
// 않는다. 유료 대안(테서 등)이 거론되지만 공식 배포처를 확인하지 못해 카드로 넣지 않았다.
// 필요해지면 그때 커뮤니티에서 현재 살아 있는 방법을 확인하는 편이 맞다.
